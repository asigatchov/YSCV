...
fn ensure_nhwc(gpu: &GpuBackend, name: &str, gc: &mut GpuCache) {
    let needs = gc
        .get(name)
        .is_some_and(|gt| !gt.nhwc && gt.buf.shape().len() == 4);
    if needs {
...
fn get_small_i64_vec(gpu: &GpuBackend, name: &str, env: &TensorEnv, gc: &GpuCache) -> Vec<i64> {
    if let Some(t) = env.get(name) {
