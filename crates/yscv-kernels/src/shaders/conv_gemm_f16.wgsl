// Fused conv2d with FULL f16 compute: implicit im2col + tiled GEMM + bias + activation.
// Input/Weight/Bias/Output are f32 buffers. All shared memory AND accumulation in f16.
// f16 FMA is 2x throughput on Apple Silicon GPUs.
enable f16;
...
fn main(
    ...
    // Full f16 accumulators — 2x FMA throughput on Apple Silicon.
    ...
    for (var t = 0u; t < tiles; t++) {
        ...
        // Load A tile — f32 → f16 conversion on load.
    ...
    // Write results: convert f16 accumulators → f32 + bias + activation.
    let c = col0 + tx * TN;
    if (c + 3u < p.n_out) {
