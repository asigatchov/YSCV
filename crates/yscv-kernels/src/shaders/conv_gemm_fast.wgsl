// Optimized conv_gemm: same tile sizes as f16 variant, but uses bitwise ops
// instead of div/mod (avoids naga_div/naga_mod zero-check wrappers) and
// fully unrolls the inner compute loop (avoids naga loop-bound overhead).
...
// BK=16 → shift 4, mask 15. BN=64 → shift 6, mask 63.
const BM: u32 = 64u;
const BN: u32 = 64u;
const BK: u32 = 16u;
const BK_SHIFT: u32 = 4u;
const BK_MASK: u32 = 15u;
const BN_SHIFT: u32 = 6u;
const BN_MASK: u32 = 63u;
const TM: u32 = 4u;
const TN: u32 = 4u;
const SA_STRIDE: u32 = 17u;
const SB_STRIDE: u32 = 66u;
var<workgroup> sa: array<f16, 1088>;
var<workgroup> sb: array<f16, 1056>;

// Helper: load one element from the A tile (general conv, im2col addressing).
...
fn main(
    ...
    for (var t = 0u; t < tiles; t++) {
        let k0 = t << BK_SHIFT;  // t * BK

        // Load A tile: 4 elements per thread (64*16 / 256 = 4).
        // Use bitwise shift/mask instead of div/mod.
        let a0 = tid;
        let a1 = tid + 256u;
        let a2 = tid + 512u;
        let a3 = tid + 768u;
        ...
        if (is_1x1) {
            // Unrolled loads for 1x1 conv (simple addressing)
            {
                let r = a0 >> BK_SHIFT;
                let c = a0 & BK_MASK;
                let gr = row0 + r;
            ...
            // Unrolled loads for general conv (im2col addressing)
            {
                let r = a0 >> BK_SHIFT;
                let c = a0 & BK_MASK;
                let gr = row0 + r;
        ...
        // Load B tile: 4 elements per thread (16*64 / 256 = 4).
        {
            let r = a0 >> BN_SHIFT;
            let c = a0 & BN_MASK;
            let gr = k0 + r;
        ...
        // GEMM inner loop: fully unrolled 16 steps (BK=16), 2 at a time = 8 pairs.
        let b_base = tx * TN;
