// Fused conv2d with FULL f16 I/O + compute: implicit im2col + tiled GEMM + bias + activation.
// Input/Weight/Output are f16 storage buffers. Bias is f32. All shared memory AND accumulation in f16.
// Halves global memory bandwidth vs f32 I/O variant.
enable f16;
...
const SB_STRIDE: u32 = 66u;   // BN + 2 (gcd(33,32)=1 for f16 bank avoidance)
var<workgroup> sa: array<f16, 1088>;  // BM * SA_STRIDE
var<workgroup> sb: array<f16, 1056>;  // BK * SB_STRIDE
...
fn main(
    ...
    for (var t = 0u; t < tiles; t++) {
        ...
        // Load A tile — direct f16 read (no conversion needed).
        ...
        // GEMM inner loop — FULL f16 FMA, 2x unrolled.
        let b_base = tx * TN;
    ...
    // Write results: f16 accumulator + f32 bias → f32 activation → f16 output.
    let c = col0 + tx * TN;
    if (c + 3u < p.n_out) {
