// Small-N f16 I/O variant: BN=32 for conv layers with OC ≤ 48.
// Input/Weight/Output are f16 storage buffers. Bias is f32.
// Same bind group layout as conv_gemm_f16_io.
enable f16;
...
fn main(
    ...
    for (var t = 0u; t < tiles; t++) {
        ...
        // Load A tile (64×16 = 1024 elements, 128 threads, 8 per thread) — direct f16 read.
        ...
        // Load B tile (16×32 = 512 elements, 128 threads, 4 per thread) — direct f16 read.
        for (var i = 0u; i < 4u; i++) {
            let idx = tid + i * WG;
            let r = idx / BN;
            let c = idx % BN;
