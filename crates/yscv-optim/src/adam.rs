...
impl Adam {
    /// Creates Adam with required learning rate.
    ...
    pub fn step(
        ...
        let state = match self.state.entry(parameter_id) {
            ...
            Entry::Vacant(entry) => entry.insert(AdamState::new(weights.shape())?),
        };
    ...
    for i in 0..len {
        unsafe {
            ...
            *vp.add(i) = v;
            let m_hat = m * bc1_inv;
            let v_hat = v * bc2_inv;
            *wp.add(i) -= lr * m_hat / (v_hat.sqrt() + epsilon);
        }
    }
}
...
unsafe fn adam_update_neon(
    ...
    while i < len {
        ...
        *mp.add(i) = m;
        *vp.add(i) = v;
        *wp.add(i) -= lr * (m * bc1_inv) / ((v * bc2_inv).sqrt() + epsilon);
        i += 1;
    }
}
